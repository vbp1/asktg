...
var (
	...
	trayIconClickMu      sync.RWMutex
	trayIconClickHandler func(button TrayIconMouseButton)
)

type TrayIconMouseButton int

const (
	TrayIconMouseButtonLeft TrayIconMouseButton = iota + 1
	TrayIconMouseButtonRight
)
...
func SetTrayIconClickHandler(handler func(button TrayIconMouseButton)) {
	trayIconClickMu.Lock()
	defer trayIconClickMu.Unlock()
	trayIconClickHandler = handler
}
...
func systrayIconClicked(button TrayIconMouseButton) {
	trayIconClickMu.RLock()
	handler := trayIconClickHandler
	trayIconClickMu.RUnlock()
	if handler == nil {
		return
	}
	go handler(button)
